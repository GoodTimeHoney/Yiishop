...
<div class="header w1210 bc mt15">
                ...
                <dd>
                    ...
                    <div class="uclist mt10">
                        <ul class="list1 fl">
                            <li><a href="">用户信息></a></li>
                            <li><a href="">我的订单></a></li>
                            <li><a href="/address/address">收货地址></a></li>
                            <li><a href="">我的收藏></a></li>
                        </ul>
    ...
    <div class="nav w1210 bc mt10">
        <!--  商品分类部分 start-->
        <div class="category fl <?=Yii::$app->controller->id."/".Yii::$app->controller->action->id==="user/index"?"":"cat1"?>"> <!-- 非首页，需要添加cat1类 -->
            <div class="cat_hd">  <!-- 注意，首页在此div上只需要添加cat_hd类，非首页，默认收缩分类时添加上off类，鼠标滑过时展开菜单则将off类换成on类 -->
                <h2>全部商品分类</h2>
                <em></em>
            </div>

            <div class="cat_bd <?=Yii::$app->controller->id."/".Yii::$app->controller->action->id==="user/index"?"":"none"?>">
                <!--一级菜单-->
                <?php foreach (\backend\models\Category::find()->where(["parend_id"=>0])->all() as $k1=>$v1):?>
                <div class="cat <?=$k1==0?"item1":""?>">
                    <h3><a href="<?=\yii\helpers\Url::to(["list","id"=>$v1->id])?>"><?=$v1->name?></a> <b></b></h3>

                    <div class="cat_detail">
                        <!--二级菜单-->
                        <?php foreach (\backend\models\Category::find()->where(["parend_id"=>$v1->id])->all() as $k2=>$v2):?>
                        <dl class="<?=$k2==0?"dl_lst":""?>">
                            <dt><a href="<?=\yii\helpers\Url::to(["list","id"=>$v2->id])?>"><?=$v2->name?></a></dt>
                            <dd>
                                <!--三级菜单-->
                                <?php foreach (\backend\models\Category::find()->where(["parend_id"=>$v2->id])->all() as $k3=>$v3):?>
